<?php
/**
 * Formapress CRM - Referent Attributes Admin Page
 *
...
<h2><?php esc_attr_e( 'Referent Attributes', 'formapress-crm' ); ?></h2>
<p><?php esc_attr_e( 'Manage dynamic attributes for referent persons. These fields will appear in the person editor and are accessible via shortcodes like [crm_referent_field_name].', 'formapress-crm' ); ?></p>

<?php
$crm_referent_attributes = get_option( 'crm_person_referent_attributes', array() );
?>

<table class="widefat" id="referent-attrib-table">
	<thead>
		<tr>
	...
	<tbody>
		<?php
		$i = 0;
		if ( ! empty( $crm_referent_attributes ) ) {
			foreach ( $crm_referent_attributes as $key => $attribute ) {
		...
		<tr valign="top" class="form-attrib-row <?php echo ( $i % 2 ? '' : 'alternate' ); ?>">
			<td scope="row" class="grab">
				<input type="hidden" class="small-text" name="crm_person_referent_attributes[<?php echo wp_kses_post( $slug ); ?>][order]"
					value="<?php echo wp_kses_post( $attr_order ); ?>" />
				<span class="dashicons dashicons-menu zform-grab"></span>
			</td>

			<td>
				<input type="text" name="crm_person_referent_attributes[<?php echo wp_kses_post( $slug ); ?>][name]"
					value="<?php echo wp_kses_post( $attribute['name'] ); ?>" />
			</td>
			<td>
					<?php $attr_type = $attribute['type']; ?>
				<select name="crm_person_referent_attributes[<?php echo wp_kses_post( $slug ); ?>][type]" class="selector_types">
			...
			<td>
					...
					switch ( $attr_type ) {
						case 'checkbox':
							echo '<input type="text" name="crm_person_referent_attributes[' . wp_kses_post( $slug ) . '][options]" placeholder="Label" value="' . wp_kses_post( $options ) . '">';
							break;

						case 'select':
						case 'radio':
							echo '<textarea name="crm_person_referent_attributes[' . wp_kses_post( $slug ) . '][options]" placeholder="Valeur|Label (une option par ligne)">' . wp_kses_post( $options ) . '</textarea>';
							break;

						case 'list':
							echo '<textarea name="crm_person_referent_attributes[' . wp_kses_post( $slug ) . '][options]" placeholder="Options for formatted list">' . wp_kses_post( $options ) . '</textarea>';
							break;
						...
						case 'download':
						default:
							echo '<input type="hidden" name="crm_person_referent_attributes[' . wp_kses_post( $slug ) . '][options]" value="' . wp_kses_post( $options ) . '"/>';
							break;
					}
					?>
			</td>
...
<p><input class="button-secondary add-attr-btn" type="button" data-table="referent-attrib-table" data-option="crm_person_referent_attributes"
