<?php
/**
 * Formapress CRM - Company Attributes Admin Page
 *
...
<h2><?php esc_attr_e( 'Company Attributes', 'formapress-crm' ); ?></h2>
<p><?php esc_attr_e( 'Manage dynamic attributes for companies. These fields will appear in the company editor and are accessible via shortcodes like [crm_company_field_name].', 'formapress-crm' ); ?></p>

<?php
$crm_company_attributes = get_option( 'crm_company_attributes', array() );
?>

<table class="widefat" id="company-attrib-table">
	<thead>
		<tr>
	...
	<tbody>
		<?php
		$i = 0;
		if ( ! empty( $crm_company_attributes ) ) {
			foreach ( $crm_company_attributes as $key => $attribute ) {
		...
		<tr valign="top" class="form-attrib-row <?php echo ( $i % 2 ? '' : 'alternate' ); ?>">
			<td scope="row" class="grab">
				<input type="hidden" class="small-text" name="crm_company_attributes[<?php echo wp_kses_post( $slug ); ?>][order]"
					value="<?php echo wp_kses_post( $attr_order ); ?>" />
				<span class="dashicons dashicons-menu zform-grab"></span>
			</td>

			<td>
				<input type="text" name="crm_company_attributes[<?php echo wp_kses_post( $slug ); ?>][name]"
					value="<?php echo wp_kses_post( $attribute['name'] ); ?>" />
			</td>
			<td>
					<?php $attr_type = $attribute['type']; ?>
				<select name="crm_company_attributes[<?php echo wp_kses_post( $slug ); ?>][type]" class="selector_types">
			...
			<td>
					...
					switch ( $attr_type ) {
						case 'checkbox':
							echo '<input type="text" name="crm_company_attributes[' . wp_kses_post( $slug ) . '][options]" placeholder="Label" value="' . wp_kses_post( $options ) . '">';
							break;

						case 'select':
						case 'radio':
							echo '<textarea name="crm_company_attributes[' . wp_kses_post( $slug ) . '][options]" placeholder="Valeur|Label (une option par ligne)">' . wp_kses_post( $options ) . '</textarea>';
							break;

						case 'list':
							echo '<textarea name="crm_company_attributes[' . wp_kses_post( $slug ) . '][options]" placeholder="Options for formatted list">' . wp_kses_post( $options ) . '</textarea>';
							break;
						...
						case 'download':
						default:
							echo '<input type="hidden" name="crm_company_attributes[' . wp_kses_post( $slug ) . '][options]" value="' . wp_kses_post( $options ) . '"/>';
							break;
					}
					?>
			</td>
